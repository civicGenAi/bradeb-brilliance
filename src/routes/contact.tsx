...
export default function ContactPage() {
  ...
  return (
    <PageShell>
      {/* HERO + floating form */}
      <section
        className="relative pt-24 pb-12 md:pb-32 overflow-hidden"
        style={{ background: "linear-gradient(135deg, #0a1628 0%, #07396c 60%, #1d3c6b 100%)" }}
      >
        <svg
        ...
        >
          <defs>
            <pattern id="cdots" width="30" height="30" patternUnits="userSpaceOnUse">
        ...
        {Array.from({ length: 12 }).map((_, i) => (
          <span
            key={i}
            className="absolute h-1 w-1 rounded-full bg-teal animate-particle"
            style={{
              left: `${(i * 47) % 100}%`,
        ...
        <div className="relative mx-auto max-w-7xl w-full px-6 lg:px-10 py-12 md:py-20 grid lg:grid-cols-[6fr_5fr] gap-12 items-start">
          ...
          <div>
              ...
              {[
                {
                  Icon: MapPin,
                  label: "Visit",
                  v: "Mbezi Beach, Dar es Salaam",
                  href: "https://maps.google.com/?q=Mbezi+Beach",
                },
                ...
                <a
                  key={label}
                  href={href}
                  className="group bg-white/5 hover:bg-white/10 border border-white/10 hover:border-gold/40 rounded-xl p-4 transition-all animate-fade-up"
                  style={{ animationDelay: `${0.5 + i * 0.1}s` }}
                >
                  <div className="h-9 w-9 rounded-lg bg-teal/15 text-teal flex items-center justify-center mb-3 group-hover:bg-gold group-hover:text-navy transition-all">
                    <Icon size={16} />
                  </div>
            ...
            {/* Quick chips */}
            <div
              className="flex flex-wrap gap-2 mt-6 animate-fade-up"
              style={{ animationDelay: "0.9s" }}
            >
              <a
                href="https://wa.me/255754230234"
                className="inline-flex items-center gap-2 rounded-full bg-soft-green text-white px-4 py-2 text-xs font-semibold hover:scale-105 transition-transform"
              >
          ...
          <div className="relative">
            ...
            <div className="relative bg-white rounded-2xl shadow-2xl p-6 md:p-8 animate-scale-in-soft">
              <div className="flex items-center justify-between mb-6">
                ...
                <span className="px-2 py-1 rounded-full bg-soft-green/15 text-soft-green text-[10px] font-bold uppercase tracking-wider">
                  24h Reply
                </span>
              </div>
              {sent ? (
                <div className="p-8 rounded-xl bg-soft-green/10 border border-soft-green/40 text-center">
                  <div className="h-14 w-14 rounded-full bg-soft-green text-white flex items-center justify-center mx-auto mb-4 text-2xl">
                    ✓
                  </div>
                ...
                >
                  <div className="grid grid-cols-2 gap-3">
                    <input
                      ...
                      required
                      className="bg-[#f5f6f8] rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-teal text-navy"
                    />
                    <input
                      placeholder="Phone"
                      type="tel"
                  ...
                  <input
                    ...
                    type="email"
                    required
                    className="w-full bg-[#f5f6f8] rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-teal text-navy"
                  />
                  <input
                  ...
                  <textarea
                    ...
                    className="w-full bg-[#f5f6f8] rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-teal text-navy resize-none"
                  />
                  <button
                    type="submit"
                    className="btn-shine w-full inline-flex items-center justify-center gap-2 rounded-lg bg-navy text-white py-3.5 font-bold uppercase tracking-[0.15em] text-xs hover:bg-teal transition-all"
                  >
                    Send Message <ArrowRight size={14} />
                  </button>
                  <p className="text-[#94a3b8] text-[11px] text-center">
                    By submitting you agree to be contacted by Bradeb.
                  </p>
                </form>
              )}
            </div>
      ...
      <section className="relative bg-white">
        <div className="mx-auto max-w-7xl px-6 lg:px-10 py-16 md:py-24 grid lg:grid-cols-2 gap-12 items-stretch">
          <div>
            ...
            <h2 className="font-heading font-extrabold text-navy text-3xl md:text-4xl mb-8">
              Come visit us in Mbezi Beach.
            </h2>
            <div className="space-y-5">
              {[
                ...
                <div
                  key={t}
                  className="group flex gap-4 p-4 rounded-xl hover:bg-[#f5f6f8] transition-colors"
                >
                  <div className="h-11 w-11 rounded-xl bg-teal/10 text-teal flex items-center justify-center shrink-0 group-hover:bg-teal group-hover:text-white transition-all">
                    <Icon size={18} />
                  </div>
                  <div>
                    <p className="label-cap text-teal mb-1">{t}</p>
                    {lines.map((l) => (
                      <p key={l} className="text-navy text-sm font-medium leading-relaxed">
                        {l}
                      </p>
                    ))}
                  </div>
          ...
          <div className="relative rounded-2xl overflow-hidden border border-navy/10 shadow-xl min-h-[420px] lg:min-h-full">
            ...
            />
            <a
              href="https://maps.google.com/?q=Mbezi+Beach+Ndumbwi+Kinondoni+Dar+es+Salaam"
              target="_blank"
              rel="noopener noreferrer"
              className="btn btn-primary btn-shine absolute bottom-4 right-4 shadow-lg"
            >
